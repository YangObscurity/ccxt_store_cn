...
class TradingMonitor:
    ...
    def add_strategy(self):
        self.cerebro.addstrategy(
            ...
            **self.config.get('monitor_params', {})
        )
    ...
    def add_datafeeds(self):
        ...
        for symbol in self.config['spot_symbols']:
            try:
                # 定义时间范围策略
                timeframes = [
                    ('1h', now_ts - 365*86400*1000, now_ts),  # 一年小时线
                    ('1m', now_ts - 30*86400*1000, now_ts)    # 一月分钟线
                ]
                ...
                for tf, start_ts, end_ts in timeframes:
                    ...
                    # 加载数据源
                    data = self._load_datafeed(symbol, tf, aligned_start, aligned_end)
                    if data and not data.dataname.empty:
                        valid_data.append(data)
                        logger.info(f"加载 {symbol} {tf} 数据 {len(data)} 条")
                
                if valid_data:
                    self.cerebro.adddata(valid_data)
                else:
    ...
    def _load_datafeed(self, symbol: str, timeframe: str, start_ts: int, end_ts: int) -> Optional[bt.feeds.PandasDirectData]:
        try:
            tf_ms = self.data_manager._timeframe_to_ms(timeframe)
            ...
            if len(data) < self.config['monitor_params']['min_history_bars'] * 0.7:
    ...
    def run_monitor_cycle(self):
        try:
            ...
            while not self._stop_event.is_set():
                if self.config.get('enable_gap_filling', False):
                    gap_thread = Thread(
    ...
    def _run_gap_filling(self):
        if not self.config['gap_fill']['enabled']:
            return
    ...
    def _sync_core_data(self):
        ...
        for symbol in self.config['spot_symbols']:
            try:
                # 强制验证数据范围
                for tf in ['1h', '1m']:
                    required_start = now_ts - (365 if tf == '1h' else 30)*86400*1000
                    if not self.data_manager.ensure_data_range(symbol, tf):
                        logger.error(f"紧急补全 {symbol} {tf} 数据")
                        self._fill_history(symbol, tf, required_start, now_ts)
